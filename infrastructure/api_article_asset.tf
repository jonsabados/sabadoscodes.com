...
resource "aws_api_gateway_resource" "article_asset_upload" {
...
resource "aws_api_gateway_method" "article_asset_upload" {
  ...
  http_method   = "POST"
  resource_id   = aws_api_gateway_resource.article_asset_upload.id
  rest_api_id   = aws_api_gateway_rest_api.api.id
}
...
resource "aws_lambda_permission" "article_asset_upload_allow_gateway_invoke" {
  ...
  source_arn = "arn:aws:execute-api:us-east-1:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.api.id}/*/POST/${aws_api_gateway_resource.article.path_part}/${aws_api_gateway_resource.article_asset_upload.path_part}"
}
